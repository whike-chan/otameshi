...
(function() {
  ...
  // ターゲット指定
  const targets = Array.from(document.querySelectorAll(".box, .eternal-anime-img"));
  const onceTargets = Array.from(document.querySelectorAll(".once-anime-img"));
  const lazyLoadTargets = Array.from(document.querySelectorAll(".lazy"));
  ...
  // １度だけ監視する
  const onceObserver = new IntersectionObserver(function(entries, object) {
    entries.forEach(function(entry) {
      // 交差したらクラス付与
  ...
  onceTargets.forEach(function(i) {
    onceObserver.observe(i);
  });
  ...
  lazyLoadTargets.forEach(function(i) {
