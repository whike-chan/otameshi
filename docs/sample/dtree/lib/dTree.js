    ...
    _createClass(TreeBuilder, [{
      ...
      value: function _update(source) {
        ...
        this.g.selectAll('.sibling')
          ...
          .enter()
          .append('path')
          .attr('class', function(vo) {
            console.log(vo);
            var cl = opts.styles.marriage;
            cl += ` ${vo.target.id}`;
            vo.target.marriageNode.parent.children.forEach(function(e) {
              if(e.data.extra && e.data.extra.zensai) {
                cl += ' is-divorced';
              }
            })
            // console.log(vo.hasOwnProperty('data'));
            // console.log(vo.hasOwnProperty('data.extra'));
            // if (vo.hasOwnProperty('data')) {
            //   return `${opts.styles.marriage} ${vo.data.zensai}`;
            // } else {
            //   return `${opts.styles.marriage}`;
            // }
            return cl;
          })
